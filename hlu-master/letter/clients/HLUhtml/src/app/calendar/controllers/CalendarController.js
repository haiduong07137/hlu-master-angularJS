(function() {
    ...
    angular.module('Hrm.Calendar').controller('CalendarController', CalendarController);
    ...
    angular.module('Hrm.Calendar').directive('fileDownload',function(){
        return{
            restrict:'A',
            scope:{
                fileDownload:'=',
    ...
    function CalendarController($http, $rootScope, $scope, $timeout, settings, modal, toastr, blockUI, bsTableAPI, utils, focus, Uploader, FileSaver, Blob, service, $cookies, constants) {
        ...
        $rootScope.hasFowardRole = $cookies.getObject(constants.cookies_user).hasFowardRole;
        $rootScope.hasChairmanCalendarRole = $cookies.getObject(constants.cookies_user).hasChairmanCalendarRole;
        $rootScope.hasManagerRole = $cookies.getObject(constants.cookies_user).hasManagerRole;

        console.log($rootScope.hasManagerRole);
        //permission
        ...
        vm.eventTypes = [
            { id: 0, name: 'Họp trực tuyến' },
            { id: 1, name: 'Họp tại phòng họp' }
        ];
        ...
        vm.getAllEnventDateView = function() {
            // if(isSearchDto){
            //     vm.dateStart = vm.eventDto.startTime;
            // }
            service.getEventPublic(vm.dateStart).then(function(data) {
                let map = {};
                if (data != null) {
                    service.getAllEnventLog().then(function(dataLog) {
                        vm.eventLog = dataLog;
                    }).then(function() {
                        vm.eventLog.forEach(el => {
                            if (map[el.eventId]) {
                                map[el.eventId].push(el);
                            } else {
                                map[el.eventId] = [];
                                map[el.eventId].push(el);
                            }
                                ...
                                for (const key in map) {
                                    ...
                                    if (e.id == key) {
                                        for (let i = 0; i < map[key].length; i++) {
                                            if (e.room.id != map[key][i].room.id) {
                                                e.oldRoom = map[key][i].room;
                                                break;
                                            }
                                        }
                                        for (let i = 0; i < map[key].length; i++) {
                                            if (e.startTime != map[key][i].startTime) {
                                                e.oldStartTime = map[key][i].startTime;
                                                break;
                                            }
                                        }
                                        for (let i = 0; i < map[key].length; i++) {
                                            if (e.endTime != map[key][i].endTime) {
                                                e.oldEndTime = map[key][i].endTime;
                                                break;
                                            }
                                        }
                                        if (e.oldRoom == null) {
                                        ...
                                        if (e.oldStartTime == null) {
                                            e.oldStartTime = e.startTime;
                                        }
                                        if (e.oldEndTime == null) {
                                            e.oldEndTime = e.endTime;
                                        }
                                        // if (map[key].length > 1) {
                                        //     e.log = (map[key][1]);
                                        // } else if (map[key].length == 1) {
                                        //     e.log = (map[key][0]);
                                        // } else {
                                        //     e.log = {};
                                        // }
        ...
        vm.editEventDate = function(eventId) {
            ...
            service.getEventById(eventId).then(function(data) {
                if (data != null) {
                    getDepartment();
                    vm.event = data;
                    vm.chairPerson = data.chairPerson;
                    vm.isNew = false;
                    vm.isView = false;
                    vm.event = {};
                    vm.event = data;
        ...
        vm.save = function() {
            if (vm.isNew) {
                var att = {};
                if(vm.chairPerson){
                    att.displayName = vm.chairPerson.displayName;
                    att.staff = vm.chairPerson.person;
                }
                ...
                // }
                // vm.dateNow = new Date();
                // if (vm.event.startTime < vm.dateNow) {
                //     toastr.error('Thời gian sự kiện lỗi, vui lòng nhập lại.', 'Lỗi');
                //     return;
                // }
                service.saveEvent(vm.event, function successCallback(data) {
                    ...
                    if (data.status == 1) {
                        toastr.error(data.messenger, 'Thông báo');
                    } else if (data.status == 2) {
                        toastr.error(data.messenger, 'Thông báo');
                    } else if (data.status == 3) {
                        toastr.info(data.messenger, 'Thông báo');
                        // Reload users
                        vm.getAllEnventDate();
                        vm.getAllEnventDateView();
                        vm.modalInstance.close();
        ...
        vm.deleteEvent = function(eventDateId, eventId) {
            ...
            if (eventDate != null) {
                ...
                if (event != null) {
                    ...
                    if (vm.eventDelete.status == 2) {
                        toastr.error('Lịch đã được xuất bản.', 'Lỗi');
                        return;
                    }
                }
            }
        ...
        vm.cancelEvent = function(eventDateId, eventId) {
            if (!eventDateId || eventDateId < 0 || !eventId || eventId < 0) {
                toastr.error('Có lỗi xảy ra, vui lòng thử lại.', 'Lỗi');
                return;
            }
            ...
            if (eventDate != null) {
                var event = utils.findById(eventDate.events, eventId);
                if (event != null) {
                    vm.eventCanclel = event;
                    if (vm.eventCanclel.status == -1 || vm.eventCanclel.status == 4) {
                        toastr.error('Lịch đã hủy.', 'Lỗi');
                        return;
                    }
                }
            }
            ...
            vm.modalInstance.result.then(function(confirm) {
                if (confirm == 'yes') {
                    service.cancelEvent(vm.eventCanclel.id, function successCallback(data) {

                    }, function errorCallback(response) {
                        toastr.error('Có lỗi xảy ra khi hủy sự kiện.', 'Thông báo');
                    }).then(function(data) {
                        if (data.status == 1) {
                            toastr.error(data.messenger, 'Thông báo');
                        } else if (data.status == 2) {
        ...
        vm.restoreEvent = function(eventDateId, eventId) {
            if (!eventDateId || eventDateId < 0 || !eventId || eventId < 0) {
                toastr.error('Có lỗi xảy ra, vui lòng thử lại.', 'Lỗi');
                return;
            }
            ...
            if (eventDate != null) {
                var event = utils.findById(eventDate.events, eventId);
                if (event != null) {
                    vm.eventRestore = event;
                    if (vm.eventRestore.status != -1 && vm.eventRestore.status != 4) {
                        toastr.error('Lịch đã khôi phục.', 'Lỗi');
                        return;
                    }
                }
            }
            ...
            vm.modalInstance.result.then(function(confirm) {
                if (confirm == 'yes') {
                    service.restoreEvent(vm.eventRestore.id, function successCallback(data) {

                    }, function errorCallback(response) {
                        toastr.error('Có lỗi xảy ra khi khôi phục sự kiện.', 'Thông báo');
                    }).then(function(data) {
                        if (data.status == 1) {
                            toastr.error(data.messenger, 'Thông báo');
                        } else if (data.status == 2) {
                    ...
                    if (vm.eventDates[i].events != null) {
                        for (var j = 0; j < vm.eventDates[i].events.length; j++) {
                            if (vm.eventDates[i].events[j].isChecked != null && vm.eventDates[i].events[j].isChecked == true) {
                                ...
                                } else if (vm.eventDates[i].events[j].status == 0) {
                                    toastr.warning('Sự kiện: "' + vm.eventDates[i].events[j].title + '" chưa được phê duyệt.', 'Thông báo');
                                    return;
                                } else if (vm.eventDates[i].events[j].status == -1 || vm.eventDates[i].events[j].status == 4) {
                                    toastr.warning('Sự kiện: "' + vm.eventDates[i].events[j].title + '" đã hủy.', 'Thông báo');
                                    return;
                                } else {
                                    listApprove.push(vm.eventDates[i].events[j]);
                                }
                ...
                for (var i = 0; i < vm.eventDates.length; i++) {
                    if (vm.eventDates[i].events != null) {
                        for (var j = 0; j < vm.eventDates[i].events.length; j++) {
                            if (vm.eventDates[i].events[j].isChecked != null && vm.eventDates[i].events[j].isChecked == true) {
                                vm.hasCheck = true;
                                if (vm.eventDates[i].events[j].status == 1) {

                                    toastr.warning('Sự kiện: "' + vm.eventDates[i].events[j].title + '" chưa được xuất bản.', 'Thông báo');
                                    return;
                                    // listApprove.push(vm.eventDates[i].events[j]);
                                } else if (vm.eventDates[i].events[j].status == 0) {
                                    toastr.warning('Sự kiện: "' + vm.eventDates[i].events[j].title + '" chưa được phê duyệt.', 'Thông báo');
                    ...
                    if (vm.eventDates[i].events != null) {
                        for (var j = 0; j < vm.eventDates[i].events.length; j++) {
                            if (vm.eventDates[i].events[j].isChecked != null && vm.eventDates[i].events[j].isChecked == true) {
                                vm.hasCheck = true;
                                if (vm.eventDates[i].events[j].status == 1) {
                                    toastr.warning('Sự kiện: "' + vm.eventDates[i].events[j].title + '" đã được phê duyệt.', 'Thông báo');
                                    return;
                                } else if (vm.eventDates[i].events[j].status == 2) {
                                    toastr.warning('Sự kiện: "' + vm.eventDates[i].events[j].title + '" đã được xuất bản.', 'Thông báo');
                                    return;
                                } else if (vm.eventDates[i].events[j].status == -1 || vm.eventDates[i].events[j].status == 4) {
                                    toastr.warning('Sự kiện: "' + vm.eventDates[i].events[j].title + '" đã hủy.', 'Thông báo');
        ...
        service.getListTaskOwnerByRoleCode('ChairmanCalendarRole').then(function(data) {
            vm.taskOwnersChairmanCalendar = data.map(function(element) {
                if (element.person) {
        ...
        service.searchRoom(vm.RoomSearch, 99999, 1).then(function(data) {
            vm.listRoom = data.content;
            console.log(vm.listRoom);
            vm.bsTableControlRoom.options.data = vm.listRoom;
            vm.bsTableControlRoom.options.totalRows = data.totalElements;
        });
        ...
        vm.searchByText = function(){
            ...
            if(vm.eventDto.startTime != null && vm.eventDto.endTime != null){
                if((vm.eventDto.startTime).getTime() > (vm.eventDto.endTime).getTime()){
                    toastr.warning("Thời gian bắt đầu sự kiện không thể lớn hơn thời gian kết thúc!");
                }else{
                    service.getListEventBySearchDto(vm.eventDto).then(function(data){
                        if(data != null){
                            vm.eventLog = data;
                        }
                    })
        ...
        $scope.getExportEvent=function(){
            console.log('download started, you can show a wating animation');
            service.getExportEvent(vm.eventDto)
