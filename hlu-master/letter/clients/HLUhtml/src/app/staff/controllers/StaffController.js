...
(function() {
    ...
    function StaffController($rootScope, $scope, toastr, $timeout, settings, utils, modal, service, Upload, positionTitleService, departmentService, SocialClassService, agreementService, AgreementTypeService, EducationService, FamilyRelationshipService) {
        ...
        vm.newStaff = function() {
            ...
            //             toastr.warning('Vui lòng nhập mã công chức.', 'Cảnh báo');
            //             return;
            //         }
            //         if (vm.validate()) {
            //             return;
        ...
        $scope.editStaff = function(staffId) {
            ...
            service.getStaff(staffId).then(function(data) {
                ...
                //console.log(data);
                vm.staff.isNew = false;
                if (vm.staff.address != null && vm.staff.address.length > 0) {
                    vm.familyRegister = vm.staff.address[0].address;
                    vm.nowAddress = vm.staff.address[0].address1;
                }
                vm.staffId = staffId;
        ...
        vm.save = function() {
            if (vm.isAdd == true) {
                ...
                if (vm.staff.staffCode && vm.staff.staffCode.trim() != "") {
                    ...
                    service.validateStaffCode(vm.staff.staffCode, vm.staff.id).then(function(data) {
                        if (!data) {
                            toastr.warning("Mã công chức này đã tồn tại", "Thông báo");
                            return;
                        } else {
                            vm.staff.address = [];
                            vm.staff.address.push({ address: vm.familyRegister, address1: vm.nowAddress });
                            vm.staff.agreements = vm.agreements;
                            vm.staff.educations = vm.educations;
                            vm.staff.familyRelationships = vm.familyRelationships;
                            service.saveStaff(vm.staff, function success() {
                ...
                if (vm.staff.staffCode && vm.staff.staffCode.trim() != "") {
                    ...
                    service.validateStaffCode(vm.staff.staffCode, vm.staff.id).then(function(data) {
                        if (!data) {
                            ...
                            vm.staff.address = [];
                            vm.staff.address.push({ address: vm.familyRegister, address1: vm.nowAddress });
                            if ((vm.agreementListDelete != null && vm.agreementListDelete.length > 0) ||
                                (vm.educationHistoryListDelete != null && vm.educationHistoryListDelete.length > 0) ||
                                (vm.familyRelationshipListDelete != null && vm.familyRelationshipListDelete.length > 0)) {
                            ...
                            service.updateStaff(vm.staffId, vm.staff, function success() {
                                // Refresh list
                                // vm.searchByDto();
                                // Notify
                                ...
                                toastr.error('Có lỗi xảy ra khi sửa thông tin.', 'Lỗi');
                            });
                        }
                    });
                }
