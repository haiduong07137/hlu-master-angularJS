...
(function() {
    ...
    function AssignerProcessLetterInController($rootScope, $scope, toastr, $timeout, settings, utils,
        modal, service, Uploader, $state, typeService, fieldService, priorityService, securityService, taskOwnerService, letterIndocumentFactory, FileSaver, $window) {
        ...
        function getLetterInDocumentById(id) {
            service.getLetterInDocumentById(id).then(function(data) {
                ...
                if (data && data.task && data.task && data.task.participates && data.task.participates.length > 0) {
                    var userTaskOwners = false;
                    angular.forEach(data.task.participates, function(element, key) {
                        if (element.taskOwner.userTaskOwners.length > 0) {
                            angular.forEach(element.taskOwner.userTaskOwners, function(userTaskOwner, key) {
                                if(userTaskOwner.user.id == $scope.currentUser.id && userTaskOwner.role.code === "AssignerRole")
                                {
                                    console.log(userTaskOwner.user.id = $scope.currentUser.id);
                                    vm.assigner = element;
                                }
                            });
                        }
        ...
        function getAllTaskOwner() {
            ...
            //TODO
                ...
                if (vm.type === 1) {
                    ...
                    service.searchListTaskOwnerByRoleCode('ProcessRole', vm.searchTaskOwner).then(function(data) {
