...
(function() {
    ...
    function LetterManagementService($http, $q, $filter, settings, utils) {
        ...
        self.searchListTaskOwnerByRoleCode = searchListTaskOwnerByRoleCode;
        self.getLetterInDocumentById = getLetterInDocumentById;
        self.newLetterInStepAssign = newLetterInStepAssign;
        self.newLetterInStepMainAssign = newLetterInStepMainAssign;
        ...
        self.processingLetter = processingLetter;
        ...
        self.getAllLetter = getAllLetter;
        self.searchPageLetterOut = searchPageLetterOut;
        self.getExportExcelLetterIn = getExportExcelLetterIn;
        self.getExportExcelLetterOut = getExportExcelLetterOut;
        self.getCurrentUser = getCurrentUser;

        function getExportExcelLetterIn(searchLetterIntDto) {
            console.log("RUNNING");
            var deferred = $q.defer();

            $http({
                url: settings.api.baseUrl + settings.api.apiPrefix + restUrl + 'upload/exportLetterIn',
                method: "POST",//you can use also GET or POST
                data: searchLetterIntDto,
        ...
        function getExportExcelLetterOut(searchLetterOuttDto) {
            console.log("RUNNING");
            var deferred = $q.defer();

            $http({
                url: settings.api.baseUrl + settings.api.apiPrefix + restUrl + 'upload/exportLetterOut',
                method: "POST",//you can use also GET or POST
        ...
        function searchListTaskOwnerByRoleCode(roleCode, searchTaskOwner, successCallback, errorCallback) {
            var url = baseUrl + 'gettaskownerbyrolecode/' + roleCode;

            return utils.resolveAlt(url, 'POST', null, searchTaskOwner, {
                'Content-Type': 'application/json; charset=utf-8'
            }, successCallback, errorCallback);
        }
        function getAllLetter(pageIndex, pageSize) {
            var url = baseUrl + 'letter/outdocument/getAll/' + pageIndex + '/' + pageSize;
        ...
        function searchPageLetterOut(stepId, currentParticipateStates, searchDto, pageIndex, pageSize, successCallback, errorCallback) {
            var url = baseUrl + 'letter/outdocument/getletterby/' + stepId + '/' + currentParticipateStates + '/' + pageIndex + '/' + pageSize;
            return utils.resolveAlt(url, 'POST', null, searchDto, {
                'Content-Type': 'application/json; charset=utf-8'
            }, successCallback, errorCallback);
        }
        ...
        //        }
        function createLetterOut(dto, successCallback, errorCallback) {
            var url = baseUrl + 'letter/outdocument/save';
        ...
        function processingLetter(documentId, dtos, successCallback, errorCallback) {
            var url = baseUrl + restUrl + 'indocument/processingletter/' + documentId;
        ...
        function newLetterInStepMainAssign(letterIn, assignerId, successCallback, errorCallback) {
            if (angular.isUndefined(assignerId) || assignerId == null) {
                assignerId = 0;
            }
            var url = baseUrl + restUrl + 'indocument/create/mainprocess/' + assignerId;
        ...
        function getTableDefinition() {
            var _tableOperation = function(value, row, index) {
                ...
                if(row.hasFowardRole || row.hasChiefOfStaffRole){
                    str += '<a class="green-dark margin-right-10" title="Kiểm tra người đã xem" href="#" data-ng-click="$parent.checkViewDocument(' + "'" + row.id + "'" + ')"><i class="fa fa-user-circle"></i></a>'
                }
                ...
                // Giao xử lý
                if (row.task.currentStep.code === "LetterInCodeStep3" && (row.hasAssignerRole) && row.task.currentState != 2) {
                ...
                // phân luồng
                if (row.task.currentStep.code === "LetterInCodeStep2" && (row.hasFowardRole || row.hasChiefOfStaffRole)) {
            ...
            var _NullFormatter = function(value, row, index) {
                
                
            };

            var _expiredDateFormatter = function(value, row, index) {
                if (!value) {
                    if (row.statusLetter == true) {
                        return '<span class="bold" style="background: #88f57b;">[Hoàn thành]</span>';
                    } else {
                        return '';
                    }
                }
                if (row.statusLetter == true) {
                    return (moment(value).format('DD/MM/YYYY') + ' <span class="bold" style="background: #88f57b;">[Hoàn thành]</span>');
                } else {
                    if (row.expiredDateStatus == true) {
                        return (moment(value).format('DD/MM/YYYY') + ' <span class="bold" style="background: #f5f998;">[Còn ' + row.dayLeft + ' ngày]</span>');
                    } else if (row.expiredDateStatus == false) {
                        return (moment(value).format('DD/MM/YYYY') + ' <span class="bold" style="background: #fb6262;">[Quá ' + row.dayLeft + ' ngày]</span>');
                    } else if (row.expiredDateStatus == null) {
                        return (moment(value).format('DD/MM/YYYY') + ' <span class="bold" style="background: #ffd455;">[Đến hạn xử lý]</span>');
                    }
                }
            };
            ...
            var _sttNumber = function(value, row, index) {
                return index + 1;
            };
            ...
            return [{
                    ...
                    switchable: true,
                    visible: true,
                    formatter: _expiredDateFormatter,
                    cellStyle: _cellNowrap
                },
                {
                    field: 'issueOrgan',
        ...
        function getTableLetterOutDocumentDefinition() {
            ...
            var _tableOperation = function(value, row, index) {
                ...
                if (row.task.currentStep.code === 'LETTEROUTSTEP1' && row.hasClerkRole) {
                    str += '<a class="green-dark margin-right-20" title="Chỉnh sửa hồ sơ xử lý"  href="#" data-ng-click="$parent.editDocument(' + "'" + row.id + "'" + ')"><i class="icon-pencil margin-right-5"></i> </a>';
                }
                if (row.task.currentStep.code === 'LETTEROUTSTEP1' && row.hasViewPermission){
                    str += '<a class="green-dark margin-right-10" title="Xem hồ sơ xử lý" href="#" data-ng-click="$parent.viewDocument(' + "'" + row.id + "'" + ')"><img src="assets/images/letter_management/document.png" /></a>';
                }
                return str;
            };
            ...
            var _cellNowrap = function(value, row, index, field) {
                return {
                    classes: '',
                    css: { 'word-wrap': 'break-word', 'max-width':'250px' }
                };
            };
            var _cellNowrapDocOriginCode = function(){
                return {
                    classes: '',
                    css: { 'word-wrap': 'break-word','width':'20px'}
                };
            };
            var _cellNowrapAction = function(){
                return {
                    classes: '',
                    css: { 'word-wrap': 'break-word','max-width':'20px'}
                };
            };
            var _cellNowrap2 = function(value, row, index, field) {
                return {
            ...
            return [{
                ...
                },
                {
                    ...
                    visible: true,
                    formatter: _issueaOrganFormatter,
                    cellStyle: _cellNowrapDocOriginCode
                },
                {
                    field: 'briefNote',
                    title: 'Trích yếu',
