...
(function () {
    ...
    function OrganizationController($rootScope, $scope, toastr, $timeout, settings, utils, modal, service, Upload) {
        ...
        $scope.editOrganization = function (organizationId) {
            service.getOrganization(organizationId).then(function (data) {
                ...
                modalInstance.result.then(function (confirm) {
                    if (confirm == 'yes') {
                        if (!vm.organizationChild.code || vm.organizationChild.code.trim() == '') {
                            toastr.error('Vui lòng nhập mã phòng ban.', 'Lỗi');
                            return;
                        }
                        if (!vm.organizationChild.name || vm.organizationChild.name.trim() == '') {
