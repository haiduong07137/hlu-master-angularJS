...
(function () {
    ...
    function TaskOwnerService($http, $q, $filter, settings, utils) {
        ...
        self.getTaskOwnersTreeGrid = getTaskOwnersTreeGrid;
        /**
         * param : txtSearch (Tên công vị trí công việc),
         * taskOwnerId: id của công việc hiện tại (sẽ không có trong danh sách trả về)
         * pageIndex số trang hiện tại, số bản ghi có thể xuất hiện trên 1 trang.
         * tìm tất cả taskOwner theo txtSearch mà không phải là taskOwner có id bằng taskOwnerId.
         */
        self.getAllExCluding = getAllExCluding;
        ...
        function getTaskOwnersTreeGrid(pageIndex, pageSize) {
            var url = baseUrl + '/' + restUrl+'/treegrid';
            url += '/' + ((pageSize > 0) ? pageSize : 25);
            url += '/' + pageIndex;
        ...
        function getAllExCluding(txtSearch,taskOwnerId,pageIndex,pageSize, successCallback, errorCallback) {
            var url = baseUrl + restUrl+'/excluding/'+taskOwnerId+'/'+pageIndex+'/'+pageSize;
            
            return utils.resolveAlt(url, 'POST', null, [txtSearch], {
                'Content-Type': 'application/json; charset=utf-8'
            }, successCallback, errorCallback);
        }
        ...
        function getListPerson(pageIndex, pageSize) {
            var url = baseUrl + restUrl + "/get_list_person";
            url += '/' + ((pageSize > 0) ? pageSize : 25);
            url += '/' + pageIndex;
