...
//        vm.isForward=false;
        ...
        vm.newLetterIn = function () {
            ...
            vm.isView = false;//check xem ai đó đã xem chưa?
...
//                	if(vm.forwarderId != null || vm.forwarderId !=undefined){
...
//                    }
        ...
        vm.newLetterInAndBack = function () {
        	vm.letterIn.userPermission = [];
        	vm.letterIn.userPermission = vm.selectedUser.map(el=>{
            	return {user:{id: el.id, username: el.username}}
            });
...
//            }
        	...
        	if (vm.letterIn.issuedDate != null && vm.letterIn.issuedDate != '' && vm.letterIn.deliveredDate != null && vm.letterIn.deliveredDate != ''){
				if (vm.letterIn.issuedDate > vm.letterIn.deliveredDate) {
	                toastr.warning('Ngày ban hành không được lớn hơn ngày đến.', 'Thông báo');
	                return false;
				}
        	}
        	...
        	if (vm.letterIn.expiredDate != null){
        		if (((vm.letterIn.expiredDate).getTime() + 82800000) < (vm.letterIn.deliveredDate).getTime()){ //cộng thêm 82800000 = 23h để kiểm tra
            		toastr.warning('Hạn trả lời không được nhỏ hơn ngày đến.', 'Thông báo');
                    return false;
            	}
        	}
        	if (vm.letterIn.issuedDate != null && vm.letterIn.issuedDate != '' && vm.letterIn.officialDate != null && vm.letterIn.officialDate != ''){
	        	if (vm.letterIn.issuedDate > vm.letterIn.officialDate){
	        		toastr.warning('Ngày ban hành không được lớn hơn ngày hiệu lực.', 'Thông báo');
	                return false;
	        	}
        	}
        ...
        vm.task = {};
        vm.forwarder = {}; // sau thì cần phân lại theo user đăng nhập
        vm.assigner = {};
        ...
        vm.checkShowBtnComplete = function() {
			if (vm.letterIn != null && vm.letterIn.task != null && vm.letterIn.task.currentStep != null && vm.letterIn.task.currentStep.code == "LetterInCodeStep4") {
				if (vm.letterIn.hasChairmanRole || vm.letterIn.hasAssignerRole) {
					return true;
				}
			}
			return false;
