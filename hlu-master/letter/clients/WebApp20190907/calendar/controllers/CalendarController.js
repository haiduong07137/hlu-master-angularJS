...
(function () {
    ...
    function CalendarController($http,$rootScope, $scope, $timeout, settings, modal, toastr, blockUI, bsTableAPI, utils, focus,Uploader,FileSaver,Blob, service) {
        ...
        vm.eventDatesView.sort(function(a, b) {
            a = new Date(a.events.startTime);
            b = new Date(b.events.endTime);
...
//        			vm.hasCalendarPublisherPermission=data.hasCalendarPublisherPermission;
        ...
        vm.sortByTimeAsc = function (firstDate, secondDate)  {
            return firstDate.startTime - secondDate.startTime; 
        };
        ...
        vm.getAllEnventDateView = function () {
            service.getAllEnventDateView(vm.dateStart).then(function (data) {
                if (data != null) {
                    ...
                    if(vm.eventDatesView){
                        vm.eventDatesView.forEach(element => {
                            if(element.events && element.events.length>=2){
                                element.events.sort(vm.sortByTimeAsc);
                            }
                        });
                    }
